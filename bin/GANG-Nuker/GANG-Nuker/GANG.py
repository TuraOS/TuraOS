...
def spammer():
    ...
    colorama.init()
    Write.Print(f'Login Failed Due to TuraOS.\n', Colors.blue_to_purple,  interval=0.000)
    print('')
    print('')
        ...
        def main():
            ...
            async def on_ready():
                if len(gang.guilds) > 1:
                    ...
                    guildpl = "guild"
                activity = discord.Game(name=f"Wernox", type=3)
            ...
            async def nuke(ctx):
                ...
                await ctx.guild.edit(name="Heil ROG")
...
#   CHANNEL SPAMMER
    if choice == '9':
        ...
        print(f"""TURAOS FAIL. We are sorry to announce that this file cannot be in TuraOS. Error 2: No file found with the name "tokens.txt".""")
...
#   HYPESQUAD JOINER
    if choice == '12':
        ...
        print("TURAOS FAIL. We are sorry to announce that there is a file not located in the folder 'GANG-Nuker'. Error 2: No file founded named 'tokens.txt' Please contact the CEO Of TuraOS.")
        house = input("\n[\x1b[95m>\x1b[95m\x1B[37m] Choice: ")
